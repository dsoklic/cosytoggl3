...
import { changeUnmappedTaskDescription, updateTasks } from "../state/tasks";
import capitalizeTitle from "capitalize-title";
import { UpdateTaskData } from "../model/types";
...
export default function FixMappingsTable() {
  ...
  const fixMappings = () => {
    const data: UpdateTaskData[] = Object.values(unmappedTasks)
      .map(([id, { ids, totalTimeS, desc, workspace }]) => ({
        ids,
        totalTimeS,
        desc,
        workspace,
  ...
  const unmappedTasks = useSelector(
    (state: RootState) => Object.entries(state.tasks.unmappedTasks).sort(),
    shallowEqual
  );
                ...
                <td>
                  <InputGroup className="mb-3">
                    ...
                    <Button
                      ...
                      id="button-addon2"
                      onClick={() => fixCapitalization(id, desc)}
                    >
                      Aa
                    </Button>
                  </InputGroup>
